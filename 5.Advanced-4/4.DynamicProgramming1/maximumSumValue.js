...
module.exports = { 
 ...
 //return an integer
	solve : function(A, B, C, D){
        ...
        function findMaximumSumValue(arr, i , j , k) {

            if(i <= j <= k && k == N - 1) {
                return (arr[i] * B) + (arr[j] * C) + (arr[k] * D)
            }

            let ans = Number.NEGATIVE_INFINITY

            let currValue = (arr[i] * B) + (arr[j] * C) + (arr[k] * D)

            let val1 = findMaximumSumValue(arr, i+1, j+1 , k+1)

            let val2 = findMaximumSumValue(arr, i , j+1, k+1)

            let val3 = findMaximumSumValue(arr, i, j , k+1)

            return ans = Math.max(ans , val1, val2, val3, currValue)
            
        }

        return findMaximumSumValue(A, 0, 0, 0)

	}
};

//Brute force : TC (3 ^ n) SC: O(n)
